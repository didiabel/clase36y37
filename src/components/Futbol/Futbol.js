...
const Futbol = () => {
  const {futbol, guardar, setHora, ahora, setAhora} = useContext(Context)


    return ( 
        <div>
            {futbol.map(cadaCancha =>(
                ...
                <div className="card-body">
                  ...
                  {cadaCancha.horarios.map(hora => (
                       <button onClick={() => setHora(cadaCancha.nombre, hora.hora)} key={hora.id} className="btn border "> {hora.hora} </button>
                  ))}
